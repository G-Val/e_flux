...
import logging
from pprint import pformat  # Voeg pformat toe
...
class EFluxConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def authenticate(self, username, password):
        ...
        _LOGGER.debug("Sending request to E-Flux API: %s", url)
        _LOGGER.debug("Request headers: %s", pformat(headers))
        ...
        try:
            ...
            # Log de response details
            _LOGGER.debug("Response status code: %s", response.status_code)
            _LOGGER.debug("Response headers: %s", pformat(response.headers))
